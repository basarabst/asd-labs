...
void drawMultipleArc(HDC, int, int, int, int, int);                  // кратні дуги
...
void drawEdge(HDC, int, int, int**, double**, int);                  // намалювати ребро (вибірка)

void drawEdges(HDC hdc, int **coords, double **matrix, int isDirecred) {
  int n = NUM_VERTICES;
  int k = n;
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int j = 0; j < k; j++) {
      if (matrix[i][j] && i != j) {
        drawEdge(hdc, i, j, coords, matrix, isDirecred);
      }
    }
  }
...
void drawEdge(HDC hdc, int from, int to, int **coords, double **matrix, int isDirected) {
  int distance = abs(coords[0][0] - coords[0][1]);
  ...
  int absy = abs(y1 - y2);
   ...
   if (matrix[to][from] && isDirected) {
    drawMultipleArc(hdc, x1, y1, x2, y2, distance);

  } else if (y1 != START + distance && y1 == y2 && absx > distance) {
    drawUpAndDownArc(hdc, from, to, x1, y1, x2, y2, distance, isDirected);

  } else if (x1 == x2 && x1 < distance && absy > distance) {
    drawLeftArc(hdc, from, to, x1, y1, x2, y2, isDirected);

  } else if (x1 == x2 && x1 > distance * 3 && absy > distance) {
    drawRightArc(hdc, from, to, x1, y1, x2, y2, isDirected);
...
void drawMultipleArc(HDC hdc, int x1, int y1, int x2, int y2, int distance) {
  int radius = RADIUS;
  double cx, cy;
  double dx, dy;
  double fi, tanFi;
  ...
  if (y1 == y2) {
    cx = (x1 + x2) * 0.5;
    cy = (y1 + y2) * 0.5;
    indent = abs(x1 - x2) > distance ? radius * 3 : radius;
    cy = x2 > x1 ? cy + indent : cy - indent;
    dx = fabs(cx - x2);
    dy = fabs(cy - y2);
    ...
    cx = (x1 + x2) * 0.5;
    indent = abs(y1 - y2) > distance ? radius * 3 : radius;
    cx = y2 > y1 ? cx - indent : cx + indent;
    cy = (y1 + y2) * 0.5;
    dx = fabs(cx - x2);
    dy = fabs(cy - y2);
